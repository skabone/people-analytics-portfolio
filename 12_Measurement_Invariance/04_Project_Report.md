...
Meaningful cross-group comparisons in survey research depend on the assumption that respondents from different groups interpret and respond to items in the same way — a property called measurement invariance. This project tests whether the Ableist Microaggressions Scale (AMS; Conover et al., 2017), a 20-item, four-factor instrument, functions equivalently across individuals reporting mild versus severe disability-related experiences (N = 833). The invariance sequence — configural, weak, strong, and strict — was evaluated using multi-group confirmatory factor analysis in R (lavaan) and Python (semopy). Results support configural and weak invariance: the same four-factor structure holds in both groups and factor loadings are equivalent. Strong invariance fails (ΔCFI = .096), indicating that item intercepts differ systematically between groups. Practically, this means observed AMS subscale means reflect both genuine differences in experienced microaggressions and differences in how each group endorses individual items at a given latent level. Latent mean comparisons are not valid without partial invariance corrections. Researchers are advised to report subscale means separately by disability severity and to explore partial invariance models that free non-equivalent intercepts.
...
**For People Analytics practitioners:** This analysis illustrates a pattern common in employee survey work. A well-validated instrument may show systematic item-level endorsement differences across groups (e.g., tenure, function, geography) that are not attributable to genuine construct differences. Reporting group mean scores on such items without invariance testing risks drawing misleading conclusions about where the real differences lie. The MGCFA framework applied here translates directly to employee engagement, inclusion, and experience survey analytics.
